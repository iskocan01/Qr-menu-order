...
$pdf->SetFont('arial','',9);
$pdf->SetX(1);
$pdf->Cell(44,5,iconv('UTF-8','ISO-8859-9//TRANSLIT',"Nome : ".$customer->customer_full_name),0,1);
$pdf->SetX(1);
$pdf->Cell(44,5,iconv('UTF-8','ISO-8859-9//TRANSLIT',"Telefone : ".$customer->customer_number),0,1);


if ($cart[0]->order_type != "delivery") {
	$pdf->SetX(1);
	 $pdf->MultiCell(44,4,iconv('UTF-8','ISO-8859-9//TRANSLIT',"Endereço : ".$customer->customer_address." - ".$customer->customer_zip." / ".$customer->customer_mahalle),0,1);
}
...
foreach($cart as $item){
	...
	if($pdf->GetStringWidth($food[0]->title."(#)".$food[0]->id) < $cellWidth){//Burası karakter sayısının uzunluğu
		...
		$textLength = strlen($food[0]->title); // total text length
		...
		while($startChar < $textLength){ // loop until end of the text
			...
			while(
				...
				($startChar+$maxChar)< $textLength ){
					$maxChar++;
					$tmpString=substr($food[0]->title,$startChar, $maxChar); 
	...
	$pdf->SetX(1);
	$pdf->MultiCell(44,5, iconv('utf-8','ISO-8859-1//TRANSLIT',$category[0]->title), 0, 1);
	...
	$xPos = $pdf->GetX();
	$yPos = $pdf->GetY();
	$pdf->MultiCell($cellWidth, $celHeight, iconv('utf-8','ISO-8859-1',$food[0]->title),0,0 );
